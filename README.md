...
- **✉️ Cover letter generation** — one-click drafts tuned to a specific job listing and your profile.
- **🗂️ Kanban application tracker** — `Applied → Interview → Offer → Rejected` with todos and calendar events per application.
...
6. **Track it** at `/tracker` — drag an application from `Applied` → `Interview` → `Offer` / `Rejected` on the Kanban board. Todos and calendar events update in place.
