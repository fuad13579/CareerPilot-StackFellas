...
CV upload, job search, fit scoring, assistant chat, cover-letter generation,
tracker updates, todos, calendar events, and RAG status.
...
- Deployed API base: `http://104.211.90.209:8000/api`
- Deployed Swagger UI: `http://104.211.90.209:8000/docs`

## Endpoint Index

| Route Group | Main Endpoints | Purpose |
| --- | --- | --- |
| CV | `POST /api/cv/upload`, `GET /api/cv/{cv_id}/sections` | Upload and inspect processed CV content |
| Jobs | `GET /api/jobs/search`, `GET /api/jobs/recommend` | Live job search and personalized recommendations |
| Fit | `POST /api/fit/score`, `POST /api/fit/skills/score` | Programmatic fit scoring |
| Assistant | `POST /api/assistant/query`, `GET /api/assistant/history` | CV-grounded assistant with session memory |
| Cover Letter | `POST /api/cover-letter/generate` | Personalized cover-letter drafting |
| RAG | `GET /api/rag/status`, `POST /api/rag/build`, `POST /api/rag/retrieve` | RAG status and retrieval diagnostics |
| Tracker | `POST /api/tracker/applications`, `GET /api/tracker/applications`, `PATCH /api/tracker/applications/{application_id}/status`, `DELETE /api/tracker/applications/{application_id}` | Application tracker and Kanban state |
| Todos | `POST /api/todos`, `GET /api/todos`, `GET /api/todos/stats`, `PATCH /api/todos/{todo_id}`, `DELETE /api/todos/{todo_id}` | Productivity todo workflow |
| Calendar | `POST /api/calendar/events`, `GET /api/calendar/events`, `GET /api/calendar/events/{event_id}`, `PATCH /api/calendar/events/{event_id}`, `DELETE /api/calendar/events/{event_id}` | Deadline calendar workflow |
...
CareerPilot exposes a conversational assistant that uses the uploaded CV, local retrieval, and persisted session history.
...
```json
{
  ...
  "question": "Am I ready for this backend role?"
}
```

Optional fields:

- `job_id` for a tracked application context
- `job_context` for raw live-job or pasted-job context
...
- `provider`
- `fallback_used`
- `job_results` when the assistant entered job-search mode

History example:

```http
GET /api/assistant/history?session_id=session-demo-123&user_id=cp-local-user-123
```

The assistant endpoints require the same `x-careerpilot-user-id` header used by other personalized routes.
...
```json
{
  ...
  "job_description": "Looking for a Python and FastAPI intern with SQL experience."
}
```
...
The tracker powers the Kanban board on `/tracker`.
...
Minimal calendar-event create body:
...
## RAG Status and Diagnostics

Useful endpoints:

- `GET /api/rag/status?cv_id={cv_id}`
- `POST /api/rag/build`
- `POST /api/rag/retrieve`

The upload flow usually builds the local index automatically. These routes are mainly useful for debugging, validation, and judge inspection.
